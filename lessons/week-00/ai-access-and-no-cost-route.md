# AI Access — Required Subscriptions and the Fallback Routes

## Required subscriptions

This course requires the two AI subscriptions named in the syllabus: an OpenAI subscription
with **Codex access** and a Google subscription with **Gemini Pro model access**. The syllabus
is the authoritative statement of the requirement. Before the first lab, sign in to each
platform and confirm you can reach those two capabilities; tier names change periodically, and
the capability — not the tier's current name — is what matters. Anything above the required
tiers is your own call. The course assigns no textbook; these subscriptions are the course
materials.

**How the two systems are used:** ChatGPT (with Codex for coding work) is the course's main
working tool all semester. Gemini is the second opinion and the checker — when a lab asks for
two systems, you send the same request to both and treat disagreement as information. Neither
system's answer is ever evidence by itself.

Never enter private student information, credentials, licensed data, or confidential company
information into any AI system.

## Fallback routes — outages, quota limits, accessibility, and device failure

A subscription does not guarantee a working session on a given day. When a platform is down,
a quota is exhausted mid-lab, an interface is inaccessible to you, or your device fails, use
the fallback that works today. You do not need to disclose why you are using a fallback route.

1. **Published text packet:** use the published packet (for Week 1,
   `Week 1 — Dual-System Output Packet`) for the required two-system comparison.
2. **Python or device blocker:** use the published Colab/offline starter and record the exact
   symptom, fallback, owner, and next action. The instructional team will route loaner or
   campus-lab support privately; do not represent a failed setup as working.
3. **Course-run probe queue:** described below, when neither platform is reachable.

Fallback days use the same rubric and checkout fields as ordinary days. No criterion awards
points for a model name, response length, or which access route a given day required.

## Student-authored follow-up probe

The packet is evidence to interrogate, not an answer key. After reading both responses:

1. write one follow-up question that tests a disagreement, common dependency, cutoff, missing-data
   treatment, or decision boundary;
2. predict in one sentence what a defensible answer should contain;
3. run the probe in an accessible free interface when available; otherwise begin the Lab 01
   AI-use field with `COURSE-RUN PROBE:` followed by the exact probe and prediction; and
4. compare the returned result with your prediction and independently decide `accept`, `modify`,
   `reject`, or `unresolved`.

For a `COURSE-RUN PROBE`, the current lab's Brightspace response is the private queue — do not
email it or post it publicly. This route exists for genuine outages, not as a service desk:
when a platform failure blocks a required output during a lab, record
`HOLD_PENDING_COURSE_RUN_OUTPUT`, preserve your human baseline and exact probe, and keep
working; the session lead addresses shared outages in class, and a course-side delay can never
reduce your score. Everything else about your setup is yours to debug — with AI, which is
precisely the skill this course teaches.

The same rule continues after Week 1. When a later lab names Codex/Gemini or asks for two
systems, that names the comparison role. Use a **task-specific packet only when the current
lab publishes one**; the Week 1 screening packet is not evidence for a later task. If your
session is blocked that day, begin the current lab's AI-use field with `COURSE-RUN PROBE:`
and continue with your human baseline — the same handling as above, in any week.

## What to retain

Retain the exact prompt, complete returned text, interface and date, your prior prediction, and
your disposition. A screenshot alone is not sufficient; preserve selectable text or an accessible
text export. AI agreement is not validation. Check a load-bearing definition or fact against the
named authoritative source and let the human decision survive or fail on evidence.

For a private access or accommodation problem, use the published support map. No one on the
teaching team will ever ask for a password or financial details.
